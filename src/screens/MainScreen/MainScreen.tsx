import { IonPage, IonContent, IonImg, IonSpinner } from '@ionic/react';
...
const MainScreen: React.FC = () => {
    ...
    const { isChecking, isOnline, checkApiHealth } = useApiHealth();

    const handleDeeplinkToRoom = async () => {
        const handlerDeeplink = CapacitorApp.addListener('appUrlOpen', (data) => {
            try {
              ...
              console.warn('Invalid deep link', data.url);
            }
          });
      
          return await handlerDeeplink;
    }

    useEffect(() => {
        const handler = handleDeeplinkToRoom();
        return () => {
    ...
    return (
        <IonPage>
            <IonContent className="main-screen-content">
                ...
                >
                    <NetworkWarning show={!isChecking && !isOnline} />
